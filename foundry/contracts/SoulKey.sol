...
contract SoulKey is ERC721, ERC2981, Ownable2Step, ReentrancyGuard, Pausable {
    ...
    mapping(bytes32 => bool) private commitmentInUse;
    ...
    function _mintNFT(
        ...
        commitmentInUse[cdCommitmentHash] = true;
    ...
    function isCommitmentInUse(bytes32 cdCommitmentHash) external view returns (bool) {
        return commitmentInUse[cdCommitmentHash];
    }

    function burnByVault(uint256 tokenId) external onlyVault {
        address tokenOwner = ownerOf(tokenId);
        bool wasSoulbound = claimTimestamp[tokenId] != 0;
        bytes32 hash = commitmentHash[tokenId];
        _burnedCount++;
        _burn(tokenId);
