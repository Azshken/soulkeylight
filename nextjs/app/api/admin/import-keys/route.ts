...
// nextjs/app/api/admin/import-keys/route.ts
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // walletAddress is now from the session — not accepted from the request body.
    const { keys: rawKeys, contractAddress, batchNotes } = body;

    if (!contractAddress || !rawKeys) {
    ...
    // Deduplicate within submitted batch (keep first occurrence)
    ...
    if (newKeyRows.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: "All duplicates — keys already exist in the database, nothing imported",
          duplicates,
          count: 0,
        },
        { status: 409 }
